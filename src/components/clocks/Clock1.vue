<template>
    <svg>
		...
		<path id="arcHour" class="clockArc" stroke-width="3" stroke-linecap="round" filter="url(#glow)" />
		<circle id="hourDot" class="clockDot" r="3" filter="url(#glow)" />
		<!-- <circle :cx="100" :cy="50" :r="50" style="--clr:#7f7f7f;"></circle> -->
		<path id="arcMinute" class="clockArc" stroke-width="3" stroke-linecap="round" filter="url(#glow)" />
		<circle id="minuteDot" class="clockDot" r="3" filter="url(#glow)" />
		<!-- <circle :cx="100" :cy="50" :r="40" style="--clr:#7f7f7f;"></circle> -->
		<path id="arcSecond" class="clockArc" stroke-width="3" stroke-linecap="round" filter="url(#glow)" />
...
<script setup>
	...
	function initTime(){
		...
		dateArray.forEach(function(number, i){
			...
			$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(1) .showingText').html(output[0])	
			$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(2) .showingText').html(output[1])	
	...
	function setTime(){
		...
		dateArray.forEach(function(number, i){
			...
			var digitOne = $('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(1) .showingText').html();	
			if (digitOne != output[0]){
				$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(1) .belowText').html(output[0]);
				$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(1) .showingText').removeClass('showingText').addClass('aboveText');
			
				setTimeout(function(){
					$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(1) .belowText').removeClass('belowText').addClass('showingText');
				},400)
				
				setTimeout(function(){
					$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(1) .aboveText').removeClass('aboveText').addClass('belowText');
				},600)
			}

			var digitTwo = $('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(2) .showingText').html();	
			if (digitTwo != output[1]){
				$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(2) .belowText').html(output[1]);
				$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(2) .showingText').removeClass('showingText').addClass('aboveText');
			
				setTimeout(function(){
					$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(2) .belowText').removeClass('belowText').addClass('showingText');
				},400)
				
				setTimeout(function(){
					$('.clockText > span:nth-child('+(i+1)+') .digitText:nth-child(2) .aboveText').removeClass('aboveText').addClass('belowText');
				},600)
			}
...
.showingText, .belowText, .aboveText{
	display: block;
	position: absolute;
	top: 0;
	left: 0;
...
.aboveText{
	...
	transition: transform 200ms cubic-bezier(0.600, -0.280, 0.735, 0.045);
}
.clockArc {
	fill: none;
	stroke: #000000;
